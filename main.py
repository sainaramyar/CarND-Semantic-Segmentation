...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    input41 = tf.layers.conv2d_transpose(conv_1x1_7, num_classes, 4, strides=(2, 2), padding = 'same', kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
    input4 = tf.add(input41, conv_1x1_4)
    input31 = tf.layers.conv2d_transpose(input4, num_classes, 4, strides=(2, 2), padding = 'same', kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
    input3 = tf.add(input31, conv_1x1_3)
    Output = tf.layers.conv2d_transpose(input3, num_classes, 16, strides=(8, 8), padding = 'same', kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
    
    # Now upsample by 2 input = output, for all vgg layers  (ref to decoder course - motice the padding and regularizer)
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    # logits = tf.reshape(input, (-1, num_classes)) also use Adam optimizer
